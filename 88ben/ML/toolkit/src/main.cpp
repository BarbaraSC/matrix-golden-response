...
SupervisedLearner* getLearner(string model, Rand& r)
{
	...
	// else if (model.compare("backprop") == 0)
		// return new BackPropogationLearner(r);
	// else if (model.compare("neuralnet") == 0)
	// 	return new NeuralNetLearner(r);
	else if (model.compare("nn") == 0)
		return new NNLearner(r);
	else if (model.compare("backprop") == 0)
		return new BackPropogationLearner(r);
...
void doit(ArgParser& parser)
{
	...
	// Display some values
	// cout << "\nDataset name: " << fileName << endl;
	cout << "\nNumber of patterns: " << dataset.rows() << endl;
	cout << "Number of features: " << dataset.cols() << endl;
